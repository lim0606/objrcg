...
#pragma omp parallel for private(t)
  for (t = 0; t < stride*nframe; t++)
  {
    ...
    for (d = 0; d < dim; d++)
    {
      ...
      output_ptr[d*stride] = input_ptr[d*stride] - logsum;
    }
  }
...
#pragma omp parallel for private(t)
  for (t = 0; t < stride*nframe; t++)
  {
    ...
    for (d = 0; d < dim; d++) {
      ...
      sum += (accreal)gradOutput_ptr[d*stride];
    }

    for (d = 0; d < dim; d++) {
      //gradInput_ptr[d*stride] = output_ptr[d*stride] * (gradOutput_ptr[d*stride] - sum);
      gradInput_ptr[d*stride] = gradOutput_ptr[d*stride] - exp(output_ptr[d*stride]) * sum;
    }
  }
